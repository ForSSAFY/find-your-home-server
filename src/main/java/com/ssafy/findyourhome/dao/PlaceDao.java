...
//    @Select("SELECT aptCode as id, lat, lng, apartmentName as name FROM houseinfo WHERE (CAST(lat AS DOUBLE) BETWEEN #{minLat} AND #{maxLat}) AND (CAST(lng AS DOUBLE) BETWEEN #{minLng} AND #{maxLng})")
    ...
    @Select("SELECT DISTINCT sgg.sigunguCode AS id, SUBSTRING_INDEX(sgg.dongName, ' ', 1) AS name, COUNT(sgg.sigunguCode) AS cnt\n" +
            "FROM (\n" +
            "SELECT * FROM sigungu\n" +
    ...
    @Select("SELECT DISTINCT CONCAT(sgg.siCode, '000000') AS id, SUBSTRING_INDEX(sgg.sidoName, ' ', 1) AS name, COUNT(sgg.siCode) AS cnt\n" +
            "FROM (\n" +
            "SELECT * FROM sigungu\n" +
